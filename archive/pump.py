...
class Pump:
    ...
    def set_direction(self, direction: str):
        ...
        if direction in ['INF','REF'] and (new_direction[:3] == direction):
            logging.info(f'{self.name}: direction set to {direction}')
        elif direction == 'REV' and (new_direction != old_direction) and (new_direction[:3] in ['INF','REF']):
    ...
    def sleep_with_heartbeat(self, sleep_time: float, beat_interval: float = 5, error_wakeup: bool = False):
        ...
        while time.time() < end_time:
            try:
                ...
                logging.critical(f'{self.name}: Sleep was broken by error: {e}')
                raise e
